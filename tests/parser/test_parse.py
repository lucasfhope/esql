...
def test_get_parsed_query_returns_the_expected_structure(data: pd.DataFrame):
    parsedQuery = get_parsed_query(
        data=data,
        query='SELECT cust,         prod,    quant.avg,   quant.min,     g1.quant.sum,g2.state.count, g3.quant.max OVER g1,g2,g3      WHERE not cust != "DAN" and month > 5 or prod =   "APLEHSVCDGhsfjadmg_hgdoe3v¡=3h8d" Such thAt g1.year = 2020 and not g1.month    = 10,g2.state!="NY",g3.day=1 or g3.day=31 HAvING g1.quant.avg > 0.5 orDer by     1'
    )
